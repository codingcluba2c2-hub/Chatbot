...
export default function FarewellsPage() {
  ...
  const apiUrl = `${backendUrl}/api/admin/farewells`;

  const { data, isLoading } = useQuery({
    queryKey: ["farewells", search],
    queryFn: async () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["farewells"] });
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
      ...
      <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950 rounded-t-xl">
        <h2 className="text-xl font-bold tracking-tight">Farewells</h2>
                ...
                items.map((item: any) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${item.intent === 'Farewell' ? 'bg-indigo-50 text-indigo-700 border border-indigo-200' : 'bg-slate-100 text-slate-700 border border-slate-200'}`}>
                        {item.intent}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400 truncate max-w-[200px]" title={item.response}>
                      {item.response || "-"}
                    </td>
                    <td className="px-6 py-4 text-sm">
                      {item.enabled ? (
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-[450px] bg-white dark:bg-slate-900 shadow-2xl z-50 flex flex-col animate-in slide-in-from-right">
            <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
              <h3 className="text-xl font-bold tracking-tight">{editingItem ? 'Edit Farewell' : 'Create Farewell'}</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  placeholder="e.g., Polite Farewell"
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Intent <span className="text-red-500">*</span></label>
                <select 
                  value={formData.intent || "Farewell"}
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-slate-500 mb-2">These will auto-generate regex patterns.</p>
                <textarea 
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 block">Custom Regex (Optional)</label>
                <p className="text-xs text-slate-500 mb-2">Leave blank to auto-generate from aliases.</p>
                <input 
                  type="text" 
                  value={formData.regex || ''}
                  onChange={(e) => setFormData({...formData, regex: e.target.value})}
                  placeholder="e.g., (?i)\b(bye|goodbye)\b"
                  className="w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 font-mono"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 block">Response Template <span className="text-red-500">*</span></label>
                <textarea 
                  value={formData.response || ''}
                  onChange={(e) => setFormData({...formData, response: e.target.value})}
                  rows={4}
                  placeholder="e.g., Goodbye! Have a great day!"
                  className="w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                />
              </div>
            ...
            <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-end gap-3">
              ...
              >
                {saveMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4 mr-2" /> Save Rule</>}
              </button>
            </div>
          </div>
        </>
