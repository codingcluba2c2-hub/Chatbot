...
export default function FastPathsPage() {
  ...
  const apiUrl = `${backendUrl}/api/admin/fastpaths`;

  const { data, isLoading } = useQuery({
    queryKey: ["fastpaths", search],
    queryFn: async () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["fastpaths"] });
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
      ...
      <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950 rounded-t-xl">
        <h2 className="text-xl font-bold tracking-tight">FastPaths</h2>
        <div className="flex gap-3">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <input 
              type="text"
              placeholder="Search FastPaths..."
              value={search}
                ...
                items.map((item: any) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      {item.enabled ? (
                        <span className="px-2 py-1 bg-green-50 text-green-700 rounded text-xs font-medium border border-green-200">Yes</span>
                      ) : (
                        <span className="px-2 py-1 bg-slate-50 text-slate-600 rounded text-xs font-medium border border-slate-200">No</span>
                      )}
                    </td>
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-[450px] bg-white dark:bg-slate-900 shadow-2xl z-50 flex flex-col animate-in slide-in-from-right">
            <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
              <h3 className="text-xl font-bold tracking-tight">{editingItem ? 'Edit FastPath' : 'Create FastPath'}</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Trigger <span className="text-red-500">*</span></label>
                <input 
                  type="text" 
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 block">Response <span className="text-red-500">*</span></label>
                <textarea 
                  value={formData.response || ''}
                  onChange={(e) => setFormData({...formData, response: e.target.value})}
                  rows={4}
                  placeholder="e.g., I'm doing great, thank you for asking!"
                  className="w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                />
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Enabled</label>
                <select 
            ...
            <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-end gap-3">
              ...
              >
                {saveMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4 mr-2" /> Save FastPath</>}
              </button>
            </div>
          </div>
        </>
